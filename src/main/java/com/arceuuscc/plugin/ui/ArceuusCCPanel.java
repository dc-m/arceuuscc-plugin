...
public class ArceuusCCPanel extends PluginPanel {
    ...
    public ArceuusCCPanel(ArceuusCCPlugin plugin) {
        ...
        // Player info (hidden until logged in)
    ...
    private void addSectionHeader(String title, Color color) {
        ...
        eventsContainer.add(headerPanel);
    ...
    private JPanel createEventPanel(Event event) {
        ...
        // Constrain width to panel width, let height be flexible
        ...
        // Description preview and "Show Details" button
        ...
        if (description != null && !description.isEmpty()) {
            ...
            // Show Details button for long descriptions
        ...
        // Sign up / Leave button (only for UPCOMING events that haven't passed)
        if (isUpcoming) {
            // Check if event time has already passed
            boolean eventPassed = false;
            if (event.getStartTime() != null) {
                try {
                    java.time.LocalDateTime startTime = java.time.LocalDateTime.parse(event.getStartTime());
                    eventPassed = startTime.isBefore(java.time.LocalDateTime.now());
                } catch (Exception e) {
    ...
    private void showEventDetailsDialog(Event event) {
        ...
        // Description text area (scrollable) - uses emoji-compatible font
        ...
        // Show signups list if there are any
